...
export const generateD3Chart = (
  ...
  const y = d3.scaleLinear()
    .domain([0, d3.max(data, d => d.close * 1.2)])
    .range([height, 0])
  ...
  const y1 = d3.scaleLinear()
    .domain([0, d3.max(data, d => d.volume * 2)])
    .range([height, 0])
  ...
  svg.append('g')
    ...
    .call(g => g.selectAll('.tick text').attr('font-size', '1.2em').attr('fill', '#f3f4f6'))
    .append('text')
    .attr('fill', '#69a3b2')
    .attr('x', width / 2)
  ...
  function mouseMove(event) {
    ...
    const d0 = data[i - 1], // the data on the left of the mouse position
      d1 = data[i]; // the data on the right of the mouse position

    // Ensure d0 and d1 are valid objects before accessing their properties
    if (!d0 || !d1) {
      return; // or handle the error appropriately
    }

    const d = +x0 - +d0.date > +d1.date - +x0 ? d1 : d0; // choose the nearest data point

    // console.log("x0:", x0, "i:", i, "d0:", d0, "d1:", d1);
    ...
    focus
      ...
      .text(`Date:  ${d3.timeFormat('%b %d, %Y, %I:%M:%S %p')(d.date)}`)
    ...
    //   .append('tspan')
    //   .attr('font-weight', 'bold')
    //   .attr('fill', 'steel#f3f4f6')
    //   .attr('x',
    //     function () { return d3.select(this.parentNode).attr('x') })
